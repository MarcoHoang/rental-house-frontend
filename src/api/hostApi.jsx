  ...
  submitHostApplication: async (formData) => {
    try {
      ...
      if (!userId || !userEmail || !username || !nationalId) {
        throw new Error('Thiếu thông tin bắt buộc: userId, userEmail, username, nationalId');
      }
      ...
      if (idFrontPhoto) {
        ...
        try {
          const frontPhotoResponse = await hostApi.uploadFile(idFrontPhoto, 'national-id');
      ...
      if (idBackPhoto) {
        ...
        try {
          const backPhotoResponse = await hostApi.uploadFile(idBackPhoto, 'national-id');
      ...
      if (proofOfOwnership) {
        ...
        try {
          const proofResponse = await hostApi.uploadFile(proofOfOwnership, 'proof-of-ownership');
      ...
      console.log('hostApi.submitHostApplication - Response data:', response.data);
      ...
      if (error.response?.status === 400) {
        ...
        if (errorData.message) {
          ...
          throw new Error('Dữ liệu gửi không hợp lệ. Vui lòng kiểm tra lại thông tin.');
        }
      }
